...
class RestAPI {
    ...
    func postQuestion(id: String, questionTitle: String, question: String, answer: String, questionUrl: String) {
        let comment = QuestionInfo(id: id, questionTitle: questionTitle, question: question, answer: answer, questionUrl: questionUrl)
        guard let uploadData = try? JSONEncoder().encode(comment)
        else {return}

        // URL 객체 정의
        let url = URL(string: "https://helpcs.xyz")
